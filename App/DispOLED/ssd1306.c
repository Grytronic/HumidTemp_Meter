...
void SSD1306_init(uint8_t refresh, uint8_t contrast_value)
{
	...
	*/
    ...
    SSD1306_command(refresh);                            //0x80 - ratio 100 ramek/sek
    ...
    SSD1306_command(SSD1306_SETDISPLAYOFFSET);          // przesuniecie w pionie  0..31
    SSD1306_command(0x00U);
    SSD1306_command(SSD1306_SEGREMAP | 0x01);           // 0x01 - widok normalny , 0x00 - do gory nogami
...
void SSD1306_display_digit_18x26(uint8_t pos_x, uint8_t cyfra)
{
	...
	if (pos_x > (szerokosc_wyswietlacza - szerokosc_znaku))
	{
		pos_x = (szerokosc_wyswietlacza - szerokosc_znaku);
	}
